...
func init() {
	// ZenzOash mainnet Address encoding magics
	...
	// ZenzOash testnet Address encoding magics
	TestNetParams = chaincfg.TestNet3Params
	TestNetParams.Net = TestnetMagic
	TestNetParams.PubKeyHashAddrID = []byte{53}   // starting with 'x' or 'y'
	TestNetParams.ScriptHashAddrID = []byte{73}
	TestNetParams.PrivateKeyID = []byte{230}
}
...
func (p *ZenzOParser) ParseBlock(b []byte) (*bchain.Block, error) {
	...
	if h.Version > 3 {
		// Skip past AccumulatorCheckpoint which was added in zenzo block version 4
